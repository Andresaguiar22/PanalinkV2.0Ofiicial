-- =============================================================================
-- Premium 2.0 — Fase 4.7 (fix): kind 'reward' no existe en wallet_transactions
-- check. El trigger de nivel usaba 'reward' para diamantes/tickets (fuera de
-- coins) => violaba el constraint. Se usa 'reward_claim' (ya válido).
-- =============================================================================
    ...
    loop
        if r.reward_amount > 0 and not exists (
            ...
            perform public.ledger_apply(
                new.user_id,
                case when coalesce(r.reward_currency, 'coins') = 'coins' then 'event_reward' else 'reward_claim' end,
