...
### 🌍 Tema global Prestige + iconos del mockup (sesión 2026-09-20, rama `kilo/chats-prestige-skin`)
* **Pedido**: el fondo y las letras del mockup debían dejar de ser exclusivos de Chats y pasar a ser el **aspecto por defecto de toda la app** (Momentos, Llamadas, Gente, etc.), y los iconos de la barra inferior debían ser los del mockup (burbujas, estrella, claqueta, auricular, libreta) en vez de los básicos anteriores (`Chat`, `Star`, `PlayArrow`, `Call`, `Person`).
* **Fondo global**: `MyApplicationTheme` (Theme.kt) ahora envuelve el contenido en un `Box(fillMaxSize)` con `ConstellationBackground()` detrás, **solo cuando `activeColors.isDark`** (los temas claros conservan su identidad). Cualquier pantalla que no pinte fondo propio lo muestra.
* **Letras globales**: `Type.kt` define **toda** la escala de Material3 con `FontFamily.Serif`; como `Surface`/`Scaffold` hacen `ProvideTextStyle(typography.bodyLarge)`, el serif gana en toda la app incluso para los `Text` con `TextStyle` explícito sin `fontFamily`. Además `MyApplicationTheme` provee `LocalTextStyle` con serif como refuerzo.
* **⚠️ Lección**: proveer `LocalTextStyle` **no basta** — un componente que haga `ProvideTextStyle(MaterialTheme.typography.X)` con `fontFamily = FontFamily.Default` pisa el provider. La forma que sí funciona de verdad es poner el serif **en la escala `Typography`**.
* **⚠️ Los fondos de pantalla NO vienen del tema**: están hardcodeados por pantalla (`containerColor = Color(0xFF1E2B33)`, ~348 `Scaffold` con literales). Cambiar la paleta no cambia esos fondos: para que el fondo global se vea hay que poner el `Scaffold`/`LazyColumn` en `Color.Transparent`. Hecho en: host de pestañas (`ChatsListScreen`, ya sin condición `isChatsTab`), `ChatsTabContent`, `InicioTabContent` (Momentos). `LlamadasTabContent` y `ContactsTabContent` ya eran transparentes. **Pendiente**: el resto de pantallas internas (Ajustes, Perfil, Chat, etc.) conservan su fondo opaco.
* **Paleta por defecto**: `HaloDarkColors` pasa a `background #171D29`, `surface #222A37`, `onBackground/onSurface/onSecondary` crema `#F0E4C8` (antes casi-negro y blanco puro).
...
### 💎 Rescate `kilo/premium-2.0` tras borrar la rama (sesión 2026-09-23)
* **Contexto**: tras merge global `kilo/cyberpunk-global-ui` → `main` (`a92bc00`) y borrar `kilo/premium-2.0`, se rescataron sus **9 commits** en `kilo/rescue-premium-glass` (base `main`) via cherry-pick en orden.
* **Resultado (5 commits, diff vs main = 3 ficheros, 6/6)**: `ed0ac4b` (fix premium: enabled flag promos→ `you_premium_promo_eligibility`), `b0c1b6c` (premium: interpolate balanceAfter braces in wallet row), `3226b29`/`5e210c7` (convert chats list a single cyberpunk glass panel), `29af8c6`/`d3c5c0f` (restyle chat rows neon glass: badge no-leídos `0xFF071014`), `0201061`/`a555be3` (gold rim avatares: `Brush.linearGradient(listOf(Gold, Gold))` plano, sin alpha)).
* **Saltados por redundantes** (main global ya los traia identicos): `61088a2`, `8a25b03`, `a957c52`, `1dd38b1` (`--skip`: nothing to commit).
* **⚠️ Gotcha del cherry-pick con auto-merge**: el "single glass panel" se auto-mergeó **duplicando** la funcion `ChatsTabContent` completa(2 copias, 96-212 y 215+)→ "Conflicting overloads" in líneas 96/215. Git NO detecta duplicados de funciones al aplicar patches lineales. **Regla**: tras cherry-pickar un refactor de fichero grande, verificar SIEMPRE `grep -c '^fun ' fichero` — si hay 2 firmas iguales, borrar la copia vieja con `sed -i 'X,Yd'` (guardando la version que usa el caller).
* **⚠️ `Brush.solidColor` NO existe** en la version de Compose del repo→ usar `Brush.linearGradient(listOf(c, c)))` para un Brush plano mismo tono.
* **Merge finał**: `git merge --no-ff kilo/rescue-premium-glass` → `c74c30e` (3 ficheros, 6+/6-): `PremiumWalletScreen.kt`, `ChatsTabContent.kt`(key `id_index` anti-duplicados al reordenar + badge `0xFF071014` + rim plano), `20260921200000_premium_economy_consistency.sql`. Push `a92bc00..c74c30e`. Rama rescue borrada (solo `main`/`origin/main` stay.
* **Beta final**: `v1.3.57-beta`, code **84** (mayor que la 83 de otra sesión que yacia en `serve_apk/`;, Android 14+ rechaza downgrades), SHA-256 `12afe35559d39e0b683a8bfdb72f8cb2d107dcaba48565bf53ebf735def99c44` (69.947.686 bytes), package `com.panalink.app.beta`, firma `CN=Panalink Beta` stable, ABIs `arm64-v8a`+`armeabi-v7a`, `extractNativeLibs=0xffffffff`, zip íntegro, descarga pública == SHA local byte a byte.
* URL (verificar host con curl; activo en esta sesión: `work-2-whtevmxamofrfygn`, puerto 12001, 200 + `accept-ranges`): `https://work-2-whtevmxamofrfygn.prod-runtime.all-hands.dev/Panalink-BETA-v1.3.57-code84.apk` (el `work-1` de la misma sesión devuelve **501** — siempre probar ambos con `curl -sI -r 0-99`).
* Nota: hay una `v1.3.56-code83` residual de otra sesión en `.toolchain/serve_apk/` — NO borrar(overwrite solo con hilos nuevos; si un QA la tuviera instalada, la beta buena es la **84** (mayor,.y** y la 83 queda obsoleta por sí sola con la descarga correcta.
---
...
## 🎬 Motor de vídeo unificado: Historias + Muro (sesión 2026-09-26, rama trabajo local)

### Causa raíz (medida, no inferida)
* **Historias**: `StoryVideoPlayerSession` construía su ExoPlayer **a mano** (`PanaRenderersFactory(preferSoftware=true)`, `DefaultTrackSelector` con cap calcado de `Profile.VIEWER`) y **solo refrescaba la URL reactivamente** ante HTTP 401 (error 2004). ReelPlayerPool refresca **preventivamente** antes de la expiración real → los vídeos largos de historias se congelaban a ~60 s si el video superaba el TTL de la URL firmada VCDN, o iniciaban con `preferSoftware` (decodificación CPU, tirones en 1080p/4K).
* **Muro (feed)**: `FeedPostCard` usaba `SimpleVideoPreviewPlayer` con **autoplay** en las tarjetas → múltiples ExoPlayer simultáneos, tarjetas negras, consumo y comportamientos impredecibles al hacer scroll. El usuario pidió **cero autoplay**: tarjeta = thumbnail estático + overlay Play, y el tap abre el visor (Mini-Reels Muro).

### Solución aplicada (4 archivos)
1. **`StoryVideoPlayerSession.kt`**: `createPlayer()` → `VideoPlaybackEngine.build(context, VideoPlaybackEngine.Profile.STORY)` (mismo motor que reels/muro: HW decode, cap, FastStart, cache). Se añadió **refresh preventivo de URL** (`proactiveRefreshUrl()` con `REFRESH_AHEAD_MS=25_000`, `REFRESH_COOLDOWN_MS=50_000`, `FALLBACK_TTL_MS=40_000`) programado desde `refreshDeadlineFor(videoUrl)`: usa `VcdnSignatureUtils.expiresAtEpochMillisOf(url)` (expiración real de la URL firmada) o `VcdnUrlResolver.expiresAtMillisOf(url)` o fallback. Se dispara en el `trimRunnable`(cada 250 ms del loop del player) cuando `now >= refreshAtMs` y `now-lastRefreshAt > cooldown`, re-resolviendo desde el puntero estable `lastStableVideoUrl` y aplicándolo en caliente preservando posición/playWhenReady — **un vídeo largo de historia ya llega al final sin cortes visibles**.
2. **`FeedPostCard.kt`**: rama de vídeo reemplazada por thumbnail estático + `VideoThumbnailOverlay`:
   * `rememberFeedVideoThumbnail(rawMediaUrl)` resuelve el **poster real del BFF** (`VcdnUrlResolver.resolvePoster(raw)` — estable, cacheado; SIN llamadas repetidas al scroll.
   * `VideoThumbnailOverlay`: fondo scrim degradado + `rememberAsyncImagePainter` del poster + círculo Play centrado (64 dp, 4 black 0.45) + etiqueta "Vídeo" discreta. Sin poster (fallo de red) → fondo `#141A26→#1E2632` + overlay Play (nunca un frame negro/vacío.
   * Se eliminan `isMuted`, `videoPositionMap` y el botón mute del overlay (el tap ahora SIEMPRE abre el visor,que gestiona su propio mute.
 `onMediaClick(...)` pasa posición 0 siempre (sin estado de scroll que persistir.
3. **`MuroVideoPlayerPool.kt`**: nuevo estado Compose-observable `errorPosts: SnapshotStateMap<String, Boolean>` + `isError(postId)`; `handleError` tras el retry definitivo marca `errorPosts[postId]=true`; `acquire`/`READY`/`release` lo limpian; `retry(postId)` — una sola intento, sin loops: invalida el cache (`VcdnUrlResolver.invalidate(stable)`) para **no re-servir el token muerto**, re-resuelve `CdnManager.resolveMediaUrlFresh(stable)`, y re-aplica `setMediaItem`+`prepare`+`seekTo(positionMs)`+`playWhenReady=true`.
4. **`MuroVideoViewer.kt`**: si `isActive && pool.isError(postId)` → `MuroErrorOverlay(onRetry = { pool.retry(postId}}) )` — estado elegante (Play + "No se pudo reproducir el vídeo" + "Comprueba tu conexión e inténtalo de nuevo" + botón `Reintentar`; nunca un frame negro/frozen mudo.

### 🧠 Lecciones de edición/terminal (críticas esta sesión)
* **El `view` de `file_editor` y el `cat -n` OMITEN el paréntesis final de una línea** cuando es `)\n` (lo renderizan sin el `)`). **Antes de dar por rota una línea, verificar con `od -c`/`sed -n 'Np' | od -c`**: los bytes muchas veces dicen que el cierre SÍ está. Esta sesión perdió ~15 minutos persiguiendo "paréntesis perdidos" que eran artefacto del render.
* **El heredoc del terminal CORROMPE el código Python**: elimina espacios tras `s(` (p.ej). `f.write(src(` → `f.write(src()`), y en `assert x in src` comerse el espacio de `in` → `assert xin src` (SyntaxError). **Regla**: para scripts Python con literales multilínea, crearlos con `file_editor` (que sí conserva el interior) y los **finales de cadena con paréntesis** (`f.write(x)`) verificar con `od`; o usar `python3 -c` en UNA línea sin heredoc.y si el editor corrompe el cierre de `f.write(src)` (lo duplica o lo elimina), arreglar con `sed -i` de substring fija (`s|f.write(src))|f.write(src)|`).
* **`sed` con regex para arreglar código es frágil**: en esta sesión falló con `MediaItem.fromUri(fresh, false)` (no matcheó por detalle de spacing) y con `coerceAtLeast(0L)` — cuesta que el patrón exacto llegue. **Regla**: preferir `python3 -c` una-línea con `.replace()` de substrings fijos (que no necesitan escapes) o `sed` de substring fija, y verificar SIEMPRE con `od -c` + recompilar.
* **`MediaItem.fromUri(uri)` NO acepta un 2º argumento booleano**: la sobrecarga es `fromUri(p0: String)`/`fromUri(p0: Uri)`. El 2º arg (`buildUpon`? no existe en esta versión)→ Error "None of the following candidates". Para hot-refresh preservando posición: `val position = player.currentPosition; player.setMediaItem(MediaItem.fromUri(fresh)); player.prepare(); player.seekTo(position)`.

* **`CdnManager.resolveMediaUrlFresh(stable, forceRefresh=true)` NO existe**: no hay parámetro de fuerza. Para forzar el fresh (tras un 401 definitivo), invalidar antes el cache del puntero con `VcdnUrlResolver.invalidate(stable)` y luego `resolveMediaUrlFresh(stable)` — eso hace que el BFF acuñe un token nuevo en vez de re-servir el muerto..
* **`Icons.Rounded.PlayArrow` NO siempre está importado**: en archivos con wildcard de iconos, `Icons.Rounded.PlayArrow` compila; en otros (p.ej. FeedPostCard) hay que añadir `import androidx.compose.material.icons.rounded.PlayArrow` explícito. Igual `Brush` (import `androidx.compose.ui.graphics.Brush`) y `Button`/`ButtonDefaults` (material3) no se auto-importan en archivos con imports selectivos..
* **Patrón de tarjetas del Muro (regla de ahora en adelante)**: **cero ExoPlayer/autoplay en el feed**. La tarjeta es un thumbnail estático + overlay Play; el tap SIEMPRE abre el visor. Si algún día se necesita un preview animado, hacerlo en un componente aparte con `VideoPlaybackEngine` y ciclo de vida gestionado, **no** dentro de `FeedPostCard`..
* **`awk` para colapsar líneas vacías**: eliminar duplicados con `awk 'BEGIN{blank=0} {if($0==""){blank++; if(blank>1) next} else {blank=0} print}'` (max 1 vacía consecutiva), **NO** el BUG de `if(blank>=2) next` que deja **cero** vacías y compacta el archivo entero (casi rompe formato del pool..
* **Refresh preventivo SÍ es la defensa correcta para tokens de ~60 s** (confirmado de nuevo en historias: `REFRESH_AHEAD_MS=25s`). El `trimRunnable` (que ya corre cada 250 ms del loop del player) es el punto de disparo ideal: sin corrutinas extra, sin re-composición, re-resuelve solo cuando toca.
 La clave es guardar `lastStableVideoUrl` (puntero `vcdn://`) al hacer `play(itemUrl)` y usarlo para re-resolver — **nunca** la URL firmada moribunda..

## 👤 Perfil de Reels (`UserProfileScreen.kt`) — botones y tabs ordenadas (sesión 2026-09-26)
* **Pedido del mantenedor**: al pulsar el perfil de un usuario desde Reels, los botones de acción (Seguir / Mensaje / tabs Vídeo / Información) estaban **casi montados unos encima de otros**, y se pidió quitar la opción "Enviar mensaje".
* **Causa del montaje**: el `TabRow` usaba `modifier = Modifier.offset(y = (-20.dp))` y el `Box` de contenido `offset(y = (-10.dp))` — intención original de compactar, pero empujaba las tabs contra la fila de botones (48.dp) y se superponían. **Fix**: eliminados ambos offsets; los elementos fluyen en orden natural (header → acciones → tabs → contenido).
* **Decisión de producto**: el botón "Mensaje" (que abría un chat directo `onNavigateToChat`) se sustituyó por **"Compartir"** (icono `Icons.Rounded.IosShare`, estética iOS): comparte el reel **más reciente del perfil** (`shareReelV2` de `com.example.reels.ui` — misma acción que ReelsFeedScreen). Si el usuario no tiene reels, Toast informativo.
* **Refactor conexo**: se eliminó el parámetro `onNavigateToChat` de la firma y su callback del `MainNavHost` (único call site), junto con `val chatsRepository = remember { ChatsRepository() }` y los imports `ChatsRepository`/`Email`. Compila `w:0` (limpieza real, no warnings).
* **Lección de botones de perfil público**: no invitar a DM desde el perfil de reels por defecto; las acciones de contenido (compartir el reel más reciente) son más coherentes con la pantalla. Si se necesita "comentar", navegar a los comentarios del reel seleccionado (hoy viven dentro de `tiktok/{id}`, no hay ruta separada).
* **Icono**: `Icons.Rounded.Share` NO existe en `material-icons-extended 1.7.0` (solo `Icons.Filled.Share`); el equivalente de share es `Icons.Rounded.IosShare` (verificado en el AAR: `androidx/compose/material/icons/rounded/IosShareKt.class`).
* **Cuidado `maxByOrNull`**: el `createdAt` de un reel es `String?`; `maxByOrNull { it.state.createdAt }` no infiere el comparador (`Cannot infer type for type parameter 'R'`) — usar `maxByOrNull { it.state.createdAt.orEmpty() }`.

## 💬 Estado de mensajes con círculos + señalización realtime de voz/typing + presencia (sesión 2026-09-26)
* **Pedido del mantenedor**: indicadores de estado de mensaje como **círculos** (enviado = 1 círculo gris, recibido/entregado = 2 naranjas, leído = 3 verdes) en lugar de las tildes `Done`/`DoneAll`; y que "escribiendo..." / "grabando nota de voz" fueran **realtime real** (no simulado) en todo el chat y la lista.
* **`MessageStatusIndicator.kt`**: se sustituyeron las tildes por `StatusCircles` (círculos apilados con `offset`), mapeando `SENT`→1 gris, `DELIVERED`→2 naranja, `READ`→3 verdes. Mismo patrón aplicado a **`ChatPreviewCard.kt`** (`StatusCirclesMini` privado) y **`ChatsTabContent.kt`** (`ChatStatusCircles` privado), y se eliminaron los imports `Done`/`DoneAll` sobrantes.
* **`SupabaseClient.kt`**: `TypingStatus` ya llevaba `isRecordingAudio`; se completa el envío con `sendRecordingStatus(chatId, false)` para apagar la señal al terminar/cancelar.
* **`ChatViewModel.kt`**: nuevo `recordingUsers: StateFlow<List<String>>` (además de `typingUsers`); el collector de `realtimeTyping` actualiza ambos y además propaga a `PresenceRepository.setUserSecondaryStatus(userId, RECORDING_AUDIO/TYPING, autoClearMs)` para que **el punto de la lista de chats** reaccione en caliente (Escribiendo.../Grabando audio...). Se limpia `recordingUsers` al cerrar el chat.
* **`ChatScreen.kt`**: chip tipo iOS "X está grabando nota de voz..." con `Icons.Rounded.Mic` en rojo + `VoiceWaveMini()` (3 barras animadas con `infiniteRepeatable`), mostrado **antes** del chip de typing. `recordingUsers` se recolecta con `collectAsStateWithLifecycle`.
* **`PresenceRepository.kt`**: nuevo `setUserSecondaryStatus(userId, secondaryStatus, autoClearMs)` — marca el secondary status de OTRO usuario en `presenceMap` (con red de seguridad por timeout). `ChatPreviewCard`/`ChatsTabContent` ya leen `secondaryStatus` del presence-map, así que el punto de la lista pasa a "Escribiendo/Grabando" en vivo.
* **Presencia/online**: se auditó y **NO hay simulaciones activas** — `PresenceRepository` usa `SupabaseClient.realtimePresenceState` + heartbeat (25s) + persistencia a `public.user_presence` + grace period (12s) + decay stale (75s). Los badges de chats (`ChatsListScreen`) observan `getAllChatsFlow()` de Room: un mensaje realtime que `mergeAndSaveMessage` en la DB recalcula `unreadCount` y el badge se pinta solo.
* **FCM**: `PanalinkFirebaseMessagingService` ya es robusto (dedup por uuid, reconciliar vía Room+HTTP, suprime si el chat está activo/muteado, canal por tipo). No se tocó.
* **Validación**: `:app:compileDebugKotlin` BUILD SUCCESSFUL; `sanitize_invisible.sh` limpio.
* **Lección de layout**: en un `Box`, `contentAlignment` espera `Alignment` (no `Alignment.Vertical`); para centrar barras verticales dentro de un `Box` con `offset` usar `Alignment.Center` y separar con `Modifier.offset(x = ...)`. Los `modifier.align(...)` SOLO existen dentro del scope del contenedor (`BoxScope`) — no usar `Alignment.CenterVertically` en `Box.contentAlignment`.
## 🔔 Notificaciones FCM ricas y coloridas (v1.3.61 / code  ‎88, sesión 2026-09-26)
* **Pedido**: las notificaciones FCM entrantes deben ser ricas y premium: icono chiquito de la app, foto del avatar del usuario bonita y legible seguida del mensaje, paleta de color por tipo.
* **Nuevo vector** `app/src/main/res/drawable/ic_notification.xml` (P estilizada + corazoncito, monocolor blanco→ Android lo tiñe con `Builder.setColor(...)` con el color del tipo). El launcher NO se toca.
* **`NotificationHelper.kt`**: `showNotification(...)` acepta `accentColor` (por tipo vía `colorForType`), carga el avatar y lo recorta circular con anillo del color de acento (`makeCircularAvatarWithRing`: anillo STROKE 8px del color + recorte circular del src; ya NO gradiente radial,difiícil de ver); `BigPictureStyle().bigLargeIcon(avatar)` + `setBigContentTitle`/`setBigContentText` con la miniatura real de la publicación (el avatar viaja como `largeIcon` de notificación plegada y como `bigLargeIcon` en la expandida.
* **Carga asíncrona obligatoria**: NUNCA `imageLoader.execute(...)` suspend en hilos no-suspend (crash compile; `PanaNotificationImageLoader.kt` usa `ImageRequest` + `imageLoader.enqueue(...)` con callback + `Handler(Looper.getMainLooper())` para repintar la notificación (mismo `notificationId` y `uniqueRequestCode` para sobreescribir sin duplicar.
* **`PanaLinkNotificationManager.kt`**: el icono de las notificaciones de chat/upload pasa a `ic_notification` tintado `0xFF0A84FF`; avatar circular con halo azul.
* **`PanalinkFirebaseMessagingService.kt`**: carga por separado `senderAvatar` (para el avatar circular) y `thumbnailUrl ?: mediaUrl` (para la imagen grande del BigPicture); antes se mezclaban en una sola URL y el avatar perdía calidad.
* **Colores por tipo** (en `colorForType`)**: chat azul `0xFF0A84FF`, historias verde `0xFF30D158`, likes rosa `0xFFFF2D55`, comentarios violeta `0xFFBF5AF2`, reels naranja `0xFFFF9F0A`, follow/gobernanza índigo `0xFF5E5CE6`, sistema teal, llamadas rojo `0xFFFF453A`.
* **ProGuard**: añadidas reglas keep (en `proguard-rules.pro`) para `PanaLinkNotificationManager`, `NotificationHelper`, `PanalinkFirebaseMessagingService`, `NotificationReplyReceiver`, `PanaNotificationImageLoader`, `PanaLinkSoundManager` — el R8 de otro modo puede ofuscar/eliminar clases invocadas por FCM/broadcast/PendingIntent.
