...
* **Scaffold condicional**: la `TopAppBar` (estado en vivo) y el `containerColor`/`padding(paddingValues)` ahora solo se aplican si `isLiveStarted`; en pre-live el fondo es transparente y edge-to-edge, y cada elemento gestiona sus insets propios. La pantalla en vivo queda **intacta** (zero regresión).
