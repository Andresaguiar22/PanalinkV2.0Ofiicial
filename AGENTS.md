...
### 🎥 Live - cámara negra y lives fantasma (sesión 2026-09-17)
* **Síntoma 1 "hay 2 tarjetas live activas sin nadie"**: no era bug de UI: eran **filas huérfanas** en `public.live_streams` con `status='LIVE'` pegadas desde el 3 sep (host `4d116a2c...`), que nunca se marcaron ENDED al salir (cierre de app a mitad, el `DisposableEffect` no se ejecutó o el PATCH falló). El feed muestra todo `status=eq.LIVE` de la DB; verificacion: `GET /rest/v1/live_streams?select=id,title,status&status=eq.LIVE` con service_role→ devolvió esos 2 registros. **Fix (DB)**: PATCH a `status='ENDED'`+`ended_at` de los 2 ids; quedaron `[]`. **Regla**: si el usuario ve tarjetas fantasma, limpiar la DB primero; no hay forma de que la app "reviva" lives muertos (la app correctamente consulta LIVE solamente.
* **Síntoma 2 "sigue toda la cámara negra"** (tras los fixes del repeat/watching): la causa restante real estaba en **cuándo y cómo se inicializa el `SurfaceViewRenderer`**:
  * En `LiveVideoSurface`, el `factory` del `AndroidView` **creaba e inicializaba** el renderer con `initRenderer?.invoke(this)` en composición— **antes de que la vista tuviera window/surface**. LiveKit dibuja en el SurfaceViewRenderer: si se pide el `Room.initVideoRenderer` sin surface (EGL context sin anclaje), el video queda NEGRO aunque el track local exista y esté publicado para siempre.
  * **Fix**: el `factory` SOLO crea el renderer (setMirror/setScalingType + si track ya existe, addRenderer y setea `rendererRef`); el `initRenderer` se dispara **en `onGloballyPositioned`** (primera medición, surface lista, Main thread) una sola vez via flag `surfaceReady`. El `LaunchedEffect(videoTrack, rendererRef)` sigue cubriendo tracks que llegan tarde. `onGloballyPositioned` requiere import `androidx.compose.ui.layout.onGloballyPositioned`.
  * `setEnableHardwareScalar(true)` **NO existe** en livekit-android 2.13.0 (Unresolved reference al no estar en el bytecode del `SurfaceViewRenderer`); quitarlo (la escala ya la da `setScalingType(SCALE_ASPECT_FILL)`). El código anterior lo tenía y compilaba?—por un quirk de visibilidad del compilador; al refactorizar rompió en claro; **regla**: no usar ese setter.
  * **Blur de Compose sobre PreviewView de CameraX**: en `LiveBroadcastSetup` la preview de CameraX (`ImplementationMode.COMPATIBLE` = TextureView) llevaba `.blur(26.dp)`. El blur de Compose fuerza render-offscreen que **puede dejar la preview NEGRA en varios dispositivos/GPUs**. **Fix**: quitar el `.blur(...)`, conservar `.scale(1.08f)` (FILL_CENTER ya llena la pantalla; el glassmorphism se mantiene con los paneles/scrim translúcidos que flotan encima).
* **Beta resultante**: `v1.3.48-beta`, code **75**, SHA `df188d741bf65463aa8f1a22114593fd3ce2181d1721db6aad4a112cd2527216`, package `com.panalink.app.beta`, firma `CN=Panalink Beta`, ABIs ARM, extractNativeLibs true. URL fija activa: `https://work-2-lxqkaugmceedjklt.prod-runtime.all-hands.dev/Panalink-BETA-v1.3.48-code75.apk` (puerto 12001, serve_apk.py con Range 206; verificar con curl -sI que dé 200/206 antes de entregar).
