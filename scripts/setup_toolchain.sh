#!/usr/bin/env bash
# Panalink — Toolchain de build persistente para el sandbox.
#
# Instala JDK 17 (Temurin) + Android SDK (platform 35, build-tools 35/36) en
# /workspace/project/toolchain, que sobrevive a los resets del sandbox.
# Tambien fija GRADLE_USER_HOME en el workspace para no re-descargar Gradle
# en cada sesion, y genera un app/google-services.json dummy si falta.
#
# Uso:
...
TOOLCHAIN_DIR="${TOOLCHAIN_DIR:-$WORKSPACE_ROOT/toolchain}"
JDK_DIR="$TOOLCHAIN_DIR/jdk17"
SDK_DIR="$TOOLCHAIN_DIR/sdk"
GRADLE_HOME_DIR="$WORKSPACE_ROOT/gradle-home"
...
"$SDKMANAGER" --sdk_root="$SDK_DIR" \
  ...
  || { log "ERROR: sdkmanager fallo"; exit 1; }
...
# Generado por setup_toolchain.sh — source para compilar Panalink en el sandbox.
export JAVA_HOME="$JDK_DIR"
