...
class VoiceRoomViewModel(app:Application):AndroidViewModel(app){
    ...
    fun onSeatClicked(seatIndex:Int,hasRecordPermission:Boolean){val state=_uiState.value;val id=roomId?:return;hasAudioPermission=hasRecordPermission;if(seatIndex !in 0..8)return;if(seatIndex==0&&!state.isAdmin)return;val mine=state.mySeat;viewModelScope.launch{if(mine!=null){if(mine.index==seatIndex){repository.leaveSeat(id).onSuccess{refreshSnapshot(id);stopAudioLevelCapture();rtcEngine?.setMicEnabled(false);_uiState.update{it.copy(isMicEnabled=false)}}.onFailure{e->_uiState.update{it.copy(error=e.message?:"No se pudo dejar el sillón")}}}else if(state.seats.getOrNull(seatIndex)?.isOccupied!=true||state.isAdmin){stopAudioLevelCapture();repository.moveSeat(id,seatIndex).onSuccess{refreshSnapshot(id);if(hasAudioPermission){rtcEngine?.setMicEnabled(true);_uiState.update{it.copy(isMicEnabled=true)};startAudioLevelCapture()}}.onFailure{e->_uiState.update{it.copy(error=e.message?:"No se pudo cambiar de sillón")}}}}else if(state.seats.getOrNull(seatIndex)?.isOccupied!=true||state.isAdmin){repository.moveSeat(id,seatIndex).onSuccess{refreshSnapshot(id);if(hasAudioPermission){rtcEngine?.setMicEnabled(true);_uiState.update{it.copy(isMicEnabled=true)};startAudioLevelCapture()}}.onFailure{e->_uiState.update{it.copy(error=e.message?:"No se pudo ocupar el sillón")}}}}}
    ...
    fun onAudioPermissionResult(granted:Boolean){hasAudioPermission=granted;if(granted&&_uiState.value.isSeated&&!(_uiState.value.mySeat?.isMuted?:false)){rtcEngine?.setMicEnabled(true);_uiState.update{it.copy(isMicEnabled=true)};startAudioLevelCapture()}else stopAudioLevelCapture()}
    fun toggleMute(){val id=roomId?:return;val seat=_uiState.value.mySeat?:return;val muted=!seat.isMuted;viewModelScope.launch{repository.moderateMute(id,myId,muted)};rtcEngine?.setMicEnabled(!muted&&hasAudioPermission);_uiState.update{it.copy(seats=VoiceRoomSeatReducer.setMuted(it.seats,myId,muted),isMicEnabled=!muted&&hasAudioPermission)};if(muted)stopAudioLevelCapture()else if(hasAudioPermission&&_uiState.value.isSeated)startAudioLevelCapture()}
