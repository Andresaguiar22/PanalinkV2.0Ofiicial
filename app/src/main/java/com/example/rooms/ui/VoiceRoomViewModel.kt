...
class VoiceRoomViewModel(app:Application):AndroidViewModel(app){
    ...
    private suspend fun refreshSnapshot(id:String){
        ...
        repository.getMessages(id).onSuccess{dtos->val names=repository.getPublicProfiles(dtos.map{it.senderId}).getOrDefault(emptyMap());_uiState.update{it.copy(messages=(dtos.map{d->VoiceRoomMessage(d.id,d.roomId,d.senderId,names[d.senderId]?.displayName,d.content,d.createdAt))}+_uiState.value.messages.filter{it.isSystem}).distinctBy{it.id}.takeLast(VoiceRoomMessagesReducer.HISTORY_CAP))}}
    }
