...
class VoiceRoomViewModel(app:Application):AndroidViewModel(app){
    ...
    fun onSeatClicked(seatIndex:Int,hasRecordPermission:Boolean){val state=_uiState.value;val id=roomId?:return;hasAudioPermission=hasRecordPermission;if(seatIndex !in 0..8)return;if(seatIndex==0&&!state.isAdmin)return;val mine=state.mySeat;viewModelScope.launch{if(mine!=null){if(mine.index==seatIndex)repository.leaveSeat(id).onSuccess{refreshSnapshot(id);stopAudioLevelCapture()}.onFailure{e->_uiState.update{it.copy(error=e.message?:"No se pudo dejar el sillón")}} else repository.moveSeat(id,seatIndex).onSuccess{refreshSnapshot(id);if(hasAudioPermission)startAudioLevelCapture()}.onFailure{e->_uiState.update{it.copy(error=e.message?:"No se pudo cambiar de sillón")}}}else if(state.seats.getOrNull(seatIndex)?.isOccupied!=true || state.isAdmin){repository.moveSeat(id,seatIndex).onSuccess{refreshSnapshot(id);if(hasAudioPermission)startAudioLevelCapture()}.onFailure{e->_uiState.update{it.copy(error=e.message?:"No se pudo ocupar el sillón")}}}}}
